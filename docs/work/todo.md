...
- [x] Publish formal GitHub Release `v1.8.0` from `feature/host-history-retention-hooks` without merging `main`; tag `v1.8.0` points to commit `5e62d81`, and the named `lspilot-enhancer-v1.8.0.apk` online asset is verified through the GitHub API endpoint against the accepted candidate. Exact size and SHA-256 are canonical in `docs/work/findings.md`; the temporary browser-style URL propagation 404 does not make the release incomplete.
