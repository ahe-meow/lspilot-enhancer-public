...
- Current-host Smali identifies the destructive persistence seam: `repository.b.r(chatId, currentList)` serializes only the caller-provided list, while `c7.b` always calls `c7.l(chatId)` before `c7.i(rows)`—delete all, then insert the current list. Every streaming request in `va.K` starts `va$k`, which periodically reads `AiChatUiState.messages` and calls `b.r`; six additional stop/cancel/lifecycle paths also call `b.r`. Because initial UI state contains only the latest 30 rows, any such save before all old pages are loaded can permanently replace full history with a partial pagination window. The target's 76 contiguous fresh row IDs match a batch reinsertion, but the exact historical caller at 12:19 was not runtime-logged, so attribution among the seven call sites remains bounded. Current module source has no host repository or StateFlow persistence path.
