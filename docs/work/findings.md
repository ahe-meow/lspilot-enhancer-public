...
- The host chat UI uses a separate local history pager: `ka$c` triggers `va.E()` only when `firstVisibleItemIndex == 0`, `hasMoreOlder=true`, `isLoadingOlder=false`, and its in-flight marker is null. `va$e` calls `repository.b.o(chatId, oldestRowId, 30)`, whose DAO SQL is `WHERE rowId < ? ORDER BY rowId DESC LIMIT 30`; parsed results are reversed and prepended, then `ka$d` restores the scroll position. The UI filters `role=tool` rows before rendering but pagination/position math uses raw rows, so tool-heavy pages can add no visible bubbles and shift restoration. Page exceptions and per-row JSON failures are silent. On an empty/error page, message-count and loading-state keys may not change, so `ka$d` may not clear the in-flight marker and later top reaches can be blocked; this static high-probability failure still needs runtime logging.
