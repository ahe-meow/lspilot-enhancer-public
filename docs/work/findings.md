...
- The `30` limit triggers whenever the host constructs a model request: the DAO query returns the newest 30 `chat_message` rows, then the list is reversed for chronological processing before `va.x` and provider serialization. It is a read-time `LIMIT`, not a row-count deletion or periodic cleanup. Older rows remain in the host database unless another independent cleanup action removes them; they are simply absent from that request. Because the boundary is row-based rather than tool-group-aware, it can split an assistant/tool sequence. The confirmed request path does not by itself prove that the chat UI uses the same limit.
- The host chat UI uses a separate local history pager: `ka$c` triggers `va.E()` only when `firstVisibleItemIndex == 0`, `hasMoreOlder=true`, `isLoadingOlder=false`, and its in-flight marker is null. `va$e` calls `repository.b.o(chatId, oldestRowId, 30)`, whose DAO SQL is `WHERE rowId < ? ORDER BY rowId DESC LIMIT 30`; parsed results are reversed and prepended, then `ka$d` restores the scroll position. The UI filters `role=tool` rows before rendering but pagination/position math uses raw rows, so tool-heavy pages can add no visible bubbles and shift restoration. Page exceptions and per-row JSON failures are silent. On an empty/error page, message-count and loading-state keys may not change, so `ka$d` may not clear the in-flight marker and later top reaches can be blocked; this static high-probability failure still needs runtime logging. A 2026-08-18 read-only DB/WAL/SHM snapshot (`integrity_check=ok`) replayed the current latest chat as raw/visible/tool counts `30/7/23`, `30/3/27`, and `16/2/14`, proving that a successful full page can add only three visible bubbles while `ka$d` scroll math still uses a raw delta of 30; the device was locked, so this does not prove live state-marker values.
