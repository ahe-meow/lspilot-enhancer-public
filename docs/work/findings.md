...
- The host chat UI uses a separate local history pager: `ka$c` triggers `va.E()` only when `firstVisibleItemIndex == 0`, `hasMoreOlder=true`, `isLoadingOlder=false`, and its in-flight marker is null. `va$e` calls `repository.b.o(chatId, oldestRowId, 30)`, whose DAO SQL is `WHERE rowId < ? ORDER BY rowId DESC LIMIT 30`; parsed results are reversed and prepended, then `ka$d` restores the scroll position. The UI filters `role=tool` rows before rendering but pagination/position math uses raw rows. A read-only DB/WAL/SHM snapshot (`integrity_check=ok`) replayed the current 76-row chat as raw/visible/tool counts `30/7/23`, `30/3/27`, and `16/2/14`, proving a full page can add only three visible bubbles while `ka$d` uses a raw delta of 30. Live foreground gestures on the matching chat kept PID 9774, produced no visible change when pulled toward older history, changed XML/text count on the reverse scroll, and restored the exact original XML at the top; the list is responsive, but external capture cannot distinguish guard rejection, hidden successful loading, or a stale in-flight marker because the host emits no pagination state logs.
