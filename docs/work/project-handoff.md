...
- A later exact count proves the target database really contains only 76 rows (`rowId 1015816..1015891`, contiguous). The missing older history is upstream of pagination: host `repository.b.r` plus `c7.b` performs delete-all-then-insert-current-list, and the `va.K -> va$k -> va$k$a` streaming autosaver periodically persists the partial `AiChatUiState.messages` window. Saving before all old pages are loaded can permanently truncate the chat. The exact 12:19 caller was not logged, but database shape and all seven host replacement call sites are documented. Current module source has no repository/StateFlow persistence path.
