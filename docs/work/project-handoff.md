...
- Host chat UI history analysis is complete and read-only: it uses `ka$c -> va.E() -> va$e -> repository.b.o(chatId, oldestRowId, 30)`. Current DB/WAL/SHM replay confirms raw/visible/tool page counts `30/7/23`, `30/3/27`, and `16/2/14`; filtering hides most loaded rows while `ka$d` scroll math uses raw list deltas. A live foreground gesture capture on the matching 76-row chat kept PID 9774: pulling toward older history did not change the UI XML, reverse scrolling changed the XML and text count, and restoring the top reproduced the exact original XML. This rules out a frozen list but cannot distinguish guard rejection, hidden successful loading, or a stale in-flight marker without temporary runtime telemetry, which is outside the current no-runtime-modification constraint.
