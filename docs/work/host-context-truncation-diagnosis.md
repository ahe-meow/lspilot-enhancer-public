...
本轮外部只读采集仍不能读取 Compose 内存里的 `firstVisibleItemIndex`、`hasMoreOlder`、`isLoadingOlder` 或请求标记，也没有宿主自带的分页日志。因此，“顶部触发被 guard 拦截”“分页成功后被原始行数滚动恢复掩盖”和“进行中标记残留”仍无法仅凭相同 UI 哈希三选一。要直接区分，必须增加临时只读运行时遥测；当前约束禁止修改宿主或模块运行逻辑，所以文档保留这个证据边界。
...
2. **工具消息占用分页但不显示**：当前快照已证实第一页只显示 7/30 行、第一次上拉页只显示 3/30 行；DAO 成功并不等于用户能明显看到新气泡，滚动恢复还会按原始 30 行计算。
3. **数据库/解析异常被静默吞掉**：`repository.b.o` 捕获外层异常后返回空列表、`hasMoreOlder=false` 和原游标；单行 JSON 解析失败也只跳过该行，不展示错误。
4. **空页后 UI 标记可能卡住**：如果查询异常、游标无效或整页 JSON 都解析失败，列表数量和 `isLoadingOlder` 可能不变，`ka$d` 不会清除进行中标记；后续上拉会被标记条件拦截。这是当前最值得优先用运行时日志验证、但尚未现场证明的路径。
